...
let simplify_terminator (block : C.basic_block) =
  ...
  | Switch labels ->
    ...
    assert (len > 0); (* CR mshinwell: This should use [Misc.fatal_errorf] *)
    ...
    | 2 ->
      ...
      block.terminator <-
        { t with desc = Branch [t0, labels.(0); t1, labels.(1); t2, l]; }
    | _ -> ()
    end
  | _ -> ()
...
let update_predecessor's_terminators (cfg : C.t) ~pred_label
  ...
  let replace_label l =
    if l <> being_disconnected then l
    else target_label
  in
  ...
  begin match pred_block.terminator.desc with
  | Branch successors ->
  ...
  | Tailcall (Self _) ->
    ...
    assert (Label.equal being_disconnected cfg.fun_tailrec_entry_point_label);
...
let disconnect cfg_with_layout label =
  ...
  let has_more_than_one_successor =
    ...
    | _::_ -> true
  in
  if has_more_than_one_successor && has_predecessors then begin
    Misc.fatal_errorf "Cannot disconnect block %a: it has more than one \
        successor and at least one predecessor"
      Label.print label
  end;
  ...
  List.iter (fun succ ->
      ...
      succ_block.predecessors <-
        Label.Set.union (Label.Set.remove label succ_block.predecessors)
          block.predecessors)
    (Cfg.successor_labels cfg block);
  (* Update predecessor blocks. *)
  begin match Cfg.successors cfg block with
  | [_cond, target_label] ->
    Label.Set.iter (fun pred_label ->
        update_predecessor's_terminators cfg ~pred_label
          ~being_disconnected:label
          ~target_label)
      block.predecessors
  | [] | _::_ ->
  ...
  CL.set_layout cfg_with_layout ~layout:new_layout;
  (* CR mshinwell: I don't think this property will necessarily hold in
     the future. *)
  ...
  (* CR mshinwell: This next part doesn't seem to actually remove the
     trap handler blocks.  (Also, should it remove trap handlers
     transitively?) *)
  CL.filter_trap_labels cfg_with_layout
    ~f:(fun ~pushtrap_lbl -> pushtrap_lbl <> label);
  CL.remove_from_trap_depths cfg_with_layout label;
  CL.remove_from_new_labels cfg_with_layout label;
  (* CR mshinwell: The next two lines should move to e.g. [Cfg.invalidate_block]
     together with an associated test for that, perhaps.  When is such a test
     used?  If it is not used, can we skip this procedure? *)
