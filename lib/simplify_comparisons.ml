...
module Rep : sig
  ...
  (* We represent a powerset of {eq, lt, gt, uo} as a "bitvector" of length 4.
     "uo" stands for "unordered". *)
  type bit =
    | T
    | F
  ...
  let to_cfloat t : cfloat =
    ...
    (* CR mshinwell: What about ones that can't be expressed as a
       single comparison, e.g. CFle || CFge?
       Maybe return something like: Ok of ... | Cannot_simplify ? *)
    | _ -> assert false
end

(* Compute one comparison operator that is equivalent to the disjunction of
   the operators c1 and c2.  The resulting comparison is mapped through
   [f], unless the result is "Always", which is not expressible as a
   comparison. *)
...
let simplify_disjunction_float c1 c2 =
  ...
  | Always -> [C.Always]
  | Unordered -> (* CR mshinwell: or "Cannot_simplify" *)
