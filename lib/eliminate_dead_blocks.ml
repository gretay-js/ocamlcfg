...
let rec eliminate_dead_blocks cfg_with_layout =
  if CL.preserve_orig_labels cfg_with_layout then begin
    Misc.fatal_error "Won't eliminate dead blocks when \
        [preserve_orig_labels] is set"
  end;
  let found_dead =
    Label.Tbl.fold (fun label block found ->
        if block_is_dead cfg_with_layout block
        then label :: found
        else found)
  ...
  if num_found_dead > 0 then begin
    let dead_blocks =
...
let dead_blocks cfg_with_layout = eliminate_dead_blocks cfg_with_layout