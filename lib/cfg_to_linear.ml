...
(* XCR mshinwell: (following the email) If we think we can't obtain the final
   two cases in [mk_float_cond], maybe we should make this code produce an
   error in those cases? It would avoid the [must_be_last] thing entirely,
   which is a source of complexity. I'm not really convinced of the
   correctness of that part yet -- for example how do we know for sure that
   one of the Cfnlt operations generated by one of the last two cases will
   always be preceded by another comparison?

   gyorsh: simplified handling of last and also added an assert. *)

(* XCR mshinwell: Please provide example(s) of the following situation in the
   comment. *)
...
let linearize_terminator cfg (terminator : Cfg.terminator Cfg.instruction)
  ...
  let desc_list =
    ...
    | Float_test { lt; eq; gt; uo } -> (
        ...
        | 2 | 3 | 4 ->
            (* CR mshinwell for mshinwell: Re-read this part *)
            ...
            let last =
              ...
              | [] ->
                  if Label.Set.mem next.label successor_labels then
                    ...
                    (* XCR mshinwell: I think a comment here saying that the
                       next line is making an arbitrary choice would be
                       useful, this is a bit tricky to follow

                       gyorsh: there is also CR-soon at the top of this
                       function. *)
                    (* arbitrary choice *)
            ...
            (* XCR mshinwell: This code is confusing. If we have to keep the
               special handling for "last" (see CR above), can we try to make
               this more obviously correct? It's a bit dubious saying "last"
               when in fact there might be an elided jump to the next block
               after it...

               gyorsh: reworked to simplify handling of last in any case, and
               for now also fail when "must be last" is encountered. *)
        | _ -> assert false )
    | Int_test { lt; eq; gt; imm; is_signed } -> (
        ...
        | 2 | 3 ->
            ...
            in
            (* XCR mshinwell: Just checking: is [is_signed] on the next line
               definitely correct being set to false? This is probably worth
               a comment. *)
