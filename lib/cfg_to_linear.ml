...
(* CR mshinwell: (following the email) If we think we can't obtain the
   final two cases in [mk_float_test], maybe we should make this code produce
   an error in those cases?  It would avoid the [must_be_last] thing
   entirely, which is a source of complexity.  I'm not really convinced of
   the correctness of that part yet -- for example how do we know for sure
   that one of the Cfnlt operations generated by one of the last two cases
   will always be preceded by another comparison? *)

(* CR mshinwell: Please provide example(s) of the following situation in the
   comment. *)

(* Certain "unordered" outcomes of float comparisons are not expressible as
    ...
    | Float_test { lt; eq; gt; uo } -> (
        ...
        | 2 | 3 | 4 ->
            ...
            let init, target_labels =
              ...
              (* If [uo] does need a conditional jump, emit it last, because in
                 some cases its condition is not disjoint. *)
              (* CR mshinwell: This code is confusing.  If we have to keep the
                 special handling for "last" (see CR above), can we try to
                 make this more obviously correct?  It's a bit dubious saying
                 "last" when in fact there might be an elided jump to the
                 next block after it... *)
        ...
        | 2 | 3 ->
            ...
            let last =
              ...
              (* CR mshinwell: I think a comment here saying that the next
                 line is making an arbitrary choice would be useful, this is
            ...
            in
            (* XCR mshinwell: Just checking: is [is_signed] on the next line definitely
               correct being set to false?  This is probably worth a comment. *)
            (* Lcondbranch3 represents an unsigned comparison, see ocaml PR #8677 *)
