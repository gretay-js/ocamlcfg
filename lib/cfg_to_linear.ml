...
(* CR mshinwell: (following the email) If we think we can't obtain the final
   two cases in [mk_float_test], maybe we should make this code produce an
    ...
    | Float_test { lt; eq; gt; uo } -> (
        ...
        | 2 | 3 | 4 ->
            ...
            let init, target_labels =
              ...
              (* If [uo] does need a conditional jump, emit it last, because
                 in some cases its condition is not disjoint. *)
    ...
    | Int_test { lt; eq; gt; imm; is_signed } -> (
        ...
        | 2 | 3 ->
            ...
            (* XCR mshinwell: Just checking: is [is_signed] on the next line
               ...
               a comment. *)
            (* Lcondbranch3 represents an unsigned comparison, see ocaml PR
               #8677 *)
